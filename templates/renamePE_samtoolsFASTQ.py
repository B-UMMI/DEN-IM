...
def formatFastqHeaders_PE(sample_name, in_fastq_1, in_fastq_2):
...
def formartFastqHeaders_SE(sample_name, in_fastq):
    out_fastq_1 = os.path.join(os.getcwd(), sample_name + '.headersRenamed.fq')

    writer_in_fastq_1 = open(out_fastq_1, 'wt')

    outfiles = [out_fastq_1]

    with open(in_fastq, 'r') as reader_in_fastq_1:
        number_reads = 0
        plus_line = True
        quality_line = True
        for line in reader_in_fastq_1:
            writer_in_fastq_1.write(line)
...
def main(sample_id, fastq_files):
    ...
    if len(fastq_files) > 1:
        ...
        number_reads, outfiles = formatFastqHeaders_PE(sample_id, file_objects[0], file_objects[1])
        logger.info('{} read pairs were written in {}. Compressing...'.format(number_reads, outfiles))
    else:
        number_reads, outfiles = formartFastqHeaders_SE(sample_id, file_objects[0])
